  ...
  if (provider.type === "oauth") {
    try {
      ...
      try {
        ...
        if (useJwtSession) {
          const response = await fetch("https://spring-api.timeline.vg/v1/auth/login", {
            method: 'POST',
            headers: {
          ...
          const newToken = data.data.jwtToken;
          const decodedNewTokenPayload = JSON.parse(Buffer.from(newToken.split('.')[1], 'base64url').toString('utf8'));
          const cookieExpires = new Date(decodedNewTokenPayload.exp * 1000);
